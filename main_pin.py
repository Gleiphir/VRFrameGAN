...
for i,(deck, real) in enumerate(loader):
    ...
    lossDr =  criterion(D(real),torch.ones(1,1).cuda())

    lossDr.backward()

    lossDf = criterion(D(G(deck)), torch.zeros((1, 1)).cuda())
    ...
    optG.zero_grad()
    lossG = criterion(D(G(deck)),torch.ones(1,1).cuda()) #G wants D to predict 1.0
